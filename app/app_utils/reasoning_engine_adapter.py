...
Guarantees support for Vertex AI Agent Engine and Gemini Enterprise (via ADK
registration), which both invoke the engine through this contract. Agent Engine
forwards calls to ``/api/reasoning_engine`` (sync) and
...
def attach_reasoning_engine_routes(app: FastAPI) -> None:
    ...
    def resolve_method(class_method: str, *, streaming: bool) -> Any:
        rt = get_runtime()
        allowed = streaming_methods if streaming else sync_methods
        if class_method not in allowed:
            logger.warning(
                "Rejected unsupported method %r (streaming=%s). Allowed: %s",
                class_method,
                streaming,
                allowed,
            )
    ...
    async def stream_query(request: Request) -> responses.StreamingResponse:
        ...
        if not class_method:
            raise HTTPException(
                status_code=400,
                detail="Missing 'class_method' in request payload.",
            )
